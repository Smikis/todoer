...
export default function AddNewTaskView({ handleExit }) {
    ...
    async function handleConfirm() {
      if (!taskViewData.groupChosenId) {
        setTaskViewData(prev => ({ ...prev, error: TEXT.Add_New_Button.No_Group_Chosen }))
        return
      }
      ...
      if (taskViewData.sendNotification) {
        if (taskViewData.date.getTime() - Date.now() <= 0) {
            setTaskViewData(prev => ({ ...prev, error: TEXT.Validation.Date_Must_Be_In_Future }))
            return
        } else {
            res = await appendTask(taskViewData.groupChosenId, taskViewData.inputText, taskViewData.date, taskViewData.toggleRepeating)
        }
      } else {
        res = await appendTask(taskViewData.groupChosenId, taskViewData.inputText)
      }
    ...
    return (
        <>
            {taskViewData.error && (
                <Text style={styles(colors, theme).error}>
                    {taskViewData.error}
                </Text>
            )}
            <TextInput
                ...
                onChangeText={text => setTaskViewData(prev => ({ ...prev, inputText: text }))}
            ...
            <Dropdown
                ...
                selectedTextStyle={{ color: colors.Grey, fontSize: 15 }}
            ...
            <Pressable
                ...
                onPress={() => setTaskViewData(prev => ({ ...taskViewData, additionalSettingsShown: !prev.additionalSettingsShown }))}
            >
                <View
                    style={{
                        width: 30,
            ...
            <View
                style={{
                    ...
                    paddingLeft: 20,
                    paddingRight: 20,
                    display: taskViewData.additionalSettingsShown ? 'flex' : 'none',
                }}
            >
                <View
                    style={{
                ...
                >
                    ...
                    <CheckBox
                        ...
                        onValueChange={val => setTaskViewData(prev => ({ ...prev, sendNotification: val }))}
                        style={{
                            marginLeft: 'auto',
                            padding: 20,
                        }}
                ...
                {taskViewData.sendNotification ? (
                    <>
                        ...
                        <DatePicker 
                            ...
                            onDateChange={date => setTaskViewData(prev => ({ ...prev, date: date }))}
                            mode="datetime"
                        />
                        <View
                            style={{
                        ...
                        >
                            ...
                            <CheckBox
                                ...
                                onValueChange={val => setTaskViewData(prev => ({ ...prev, toggleRepeating: val }))}
                                style={{
                                    marginLeft: 'auto',
                                    padding: 20
                                }}
            ...
            <View style={styles(colors, theme).button_container}>
                <Pressable
                    ...
                    style={styles(colors, theme).button}>
                    <Text style={styles(colors, theme).button_text}>
                        {TEXT.Confirm}
                    </Text>
                </Pressable>
                <Pressable
                    onPress={handleExit}
                    style={[styles(colors, theme).button, styles(colors, theme).btn_cancel]}
                >
                    <Text
                        style={[
